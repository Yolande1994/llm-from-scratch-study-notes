...
def download_and_load_gpt2(model_size, models_dir):
    ...
    if model_size not in allowed_sizes:
        raise ValueError(f"传入的模型尺寸不在合法列表 {allowed_sizes} 中")
...
def download_file(url, destination, backup_url=None):
    ...
    def _attempt_download(download_url):
        with urllib.request.urlopen(download_url) as response:
            # 从响应头读取文件总大小，无该字段则默认赋值0
            file_size = int(response.headers.get("Content-Length", 0))

            # 校验本地文件是否已完整存在，存在则跳过下载
    ...
    try:
        ...
        if backup_url is not None:
            print(f"主下载地址({url})访问失败，尝试备用镜像地址：{backup_url}")
            try:
        ...
        # 主地址、备用地址均下载失败，输出完整错误提示
        error_message = (
            f"主地址({url})"
            f"{' 和备用地址(' + backup_url + ')' if backup_url else ''} 全部下载失败。"
            "\n请检查网络连接或确认文件资源是否可用。\n"
            "如需寻求帮助，请访问：https://github.com/rasbt/LLMs-from-scratch/discussions/273"
        )
        print(error_message)
...
def download_file(url, destination):
    # 发送GET流式请求，边下载边写入本地，避免一次性占用全部内存
    response = requests.get(url, stream=True)

    # 从响应头获取文件总大小，无该字段则默认0
    file_size = int(response.headers.get("content-length", 0))

    # 判断本地是否存在完整同名文件，存在则跳过下载
    if os.path.exists(destination):
        file_size_local = os.path.getsize(destination)
        if file_size == file_size_local:
            print(f"文件已存在且完整，无需重复下载：{destination}")
    ...
    with tqdm(total=file_size, unit="iB", unit_scale=True, desc=progress_bar_description) as progress_bar:
        ...
        with open(destination, "wb") as file:
            ...
            for chunk in response.iter_content(block_size):
                progress_bar.update(len(chunk))  # 更新进度条数值
                file.write(chunk)  # 将分块数据写入本地文件
"""


def load_gpt2_params_from_tf_ckpt(ckpt_path, settings):
    # 初始化参数字典，为每一层Transformer创建独立空字典存储权重
